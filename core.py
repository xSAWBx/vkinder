...
class VkTools:
    def __init__(self, access_token):
        ...
        self.vkapi_session = self.vkapi.get_api()
    ...
    def get_profile_info(self, user_id):
        try:
            info = self.vkapi_session.users.get(user_id=user_id, fields='city,sex,bdate,relation')
            if info:
    ...
    def search_worksheet(self, params, offset=0):
        try:
            users = self.vkapi_session.users.search(
                count=1,
                offset=offset,
    ...
    def get_photos(self, id):
        try:
            photos = self.vkapi_session.photos.get(
                owner_id=id,
                album_id='profile',
                extended=1
            )
        ...
        if photos and 'items' in photos:
            ...
            result = sorted(result, key=lambda x: (x['likes'], x['comments']), reverse=True)[:3]
